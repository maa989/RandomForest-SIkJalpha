...
%% Add US TOTAL (This applies to flusurv)
%days->states->seasons. Currenty 13384 (56*239) -> (57*239) 13623
PedILI_OldN = table;
for i=1:7
    indx = (i-1)*13384 + 1;
    temp = PedILI_Old(indx:indx+13383,:);
    if i==1
        PedILI_OldN = [temp; array2table(nansum(temp{1:239:end,:}))];
    else
        PedILI_OldN = [PedILI_OldN; temp; array2table(nansum(temp{1:239:end,:}))];
    end
    for d=2:239
        PedILI_OldN = [PedILI_OldN; array2table(nansum(temp{d:239:end,:}))];
    end
end
PedILI_Old = PedILI_OldN;
%% update table
...
old_data_no2020.Var127(old_data_no2020.Var127 == 57) = sum(popu);
old_data_no2020.Var127(isnan(old_data_no2020.Var2)) = nan;
...
for wks_back = x:-1:5
    data_train = table;
    data_test = table;
    for i=1:(size(old_data_no20202,1)/(239*(12-seasons_back))) %Basically looping over states
        data_train = [data_train; old_data_no20202(239*(i-1) + 1 +(season-1)*13623:(239*i)-(7*wks_back)+(season-1)*13623,:)];
        data_test = [data_test; old_data_no20202((239*i)-(7*wks_back) + 1+(season-1)*13623:(239*i)-(7*(wks_back-1))+(season-1)*13623,:)];
    end
    temp = old_data_no20202(1:(season-1)*13623,:);
...
%     RMSE_4(:,27-wks_back) = sqrt(nanmean((GT_4 - pred_4_m).^2)); 
    ...
    quant_preds_deaths = nan(57,4,23);
    mean_preds_deaths = nan(57,4);
    for i = 1:4
    ...
    mean_preds_deaths(57,:) = mean_preds_deaths(57,:)*sum(popu)/100000;
    quant_preds_deaths(57,:,:) = quant_preds_deaths(57,:,:)*sum(popu)/100000;

    %% Save in necessary format
    zero_date = datetime(2021, 9, 1);
    thisday = size(data_train,1)/57;
    ...
    Ti = table;
    temp = quant_preds_deaths(1:56,:,:);
    thesevals = temp(:);
    [cid, wh, qq] = ind2sub(size(quant_preds_deaths(1:56,:,:)), (1:length(thesevals))');
    ...
    Tm = table;
    temp = mean_preds_deaths(1:56,:,:);
    thesevals = temp(:);
    [cid, wh] = ind2sub(size(mean_preds_deaths(1:56,:,:)), (1:length(thesevals))');
    wh_string = sprintfc('%d', [1:max(wh)]');
    ...
    %%
    Ti = table;
    us_quants = quant_preds_deaths(57,:,:);%sum(quant_preds_deaths, 1);
    thesevals = us_quants(:);
    ...
    Tm = table;
    us_mean = mean_preds_deaths(57,:,:);%sum(mean_preds_deaths, 1);
    thesevals = us_mean(:);
