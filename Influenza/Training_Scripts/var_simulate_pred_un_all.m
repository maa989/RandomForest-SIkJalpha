function [infec] = var_simulate_pred_un_all(data_4, passengerFlowDarpa, beta_all_cell, popu, k_l, horizon, jp_l, un_fact, base_infec)
...
if length(F) <= 1    % Optimized code when mobility not considered
    for ki = 8:size(base_infec, 2)
        for j=1:length(popu)
            lastinfec = base_infec(j,ki);%end);
