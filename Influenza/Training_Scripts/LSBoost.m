...
load flu_hospitalization_TOP-New_predictorsFluSurv2
...
for wks_back = 1:1
    ...
    %Train Models
    Mdl_1 = fitrensemble([old_data_no20202(:,[1 2 3 7:36 127]);data_train(:,[1 2 3 7:36 127])],'Var3');
    Mdl_2 = fitrensemble([old_data_no20202(:,[1 2 4 37:66 127]);data_train(:,[1 2 4 37:66 127])],'Var4');
    Mdl_3 = fitrensemble([old_data_no20202(:,[1 2 5 67:96 127]);data_train(:,[1 2 5 67:96 127])],'Var5');
    Mdl_4 = fitrensemble([old_data_no20202(:,[1 2 6 97:126 127]);data_train(:,[1 2 6 97:126 127])],'Var6');
    
    %Generate mean and quantile predictions
    q = [0.01, 0.025, (0.05:0.05:0.95), 0.975, 0.99];
    predX_1 = data_test(:,[1 2 7:36 127]);
    mpgMean_1 = predict(Mdl_1,predX_1);
    for i=1:Mdl_1.NumTrained
        mpgPreds1(:,i) = predict(Mdl_1,predX_1, 'Learners', i);
    end
    mpgQuartiles_1 = quantile(mpgPreds1', q)';
    mpgQuartiles_1 = 0.5*(mpgQuartiles_1+abs(mpgQuartiles_1));

    predX_2 = data_test(:,[1 2 37:66 127]);
    mpgMean_2 = predict(Mdl_2,predX_2);
    for i=1:Mdl_2.NumTrained
        mpgPreds2(:,i) = predict(Mdl_2,predX_2, 'Learners', i);
    end
    mpgQuartiles_2 = quantile(mpgPreds2', q)';
    mpgQuartiles_2 = 0.5*(mpgQuartiles_2+abs(mpgQuartiles_2));
    
    predX_3 = data_test(:,[1 2 67:96 127]);
    mpgMean_3 = predict(Mdl_3,predX_3);
    for i=1:Mdl_3.NumTrained
        mpgPreds3(:,i) = predict(Mdl_3,predX_3, 'Learners', i);
    end
    mpgQuartiles_3 = quantile(mpgPreds3', q)';
    mpgQuartiles_3 = 0.5*(mpgQuartiles_3+abs(mpgQuartiles_3));

    predX_4 = data_test(:,[1 2 97:126 127]);
    mpgMean_4 = predict(Mdl_4,predX_4);

    for i=1:Mdl_4.NumTrained
        mpgPreds4(:,i) = predict(Mdl_4,predX_4, 'Learners', i);
    end
    mpgQuartiles_4 = quantile(mpgPreds4', q)';
    mpgQuartiles_4 = 0.5*(mpgQuartiles_4+abs(mpgQuartiles_4));
    ...
    for i=1:(7*wks_back)
        total_US_GT_1(i) = nansum(data_test.Var4(i:(7*wks_back):end));
        total_US_Mean_1(i) = nansum(mpgMean_1(i:(7*wks_back):end));
        
        total_US_GT_2(i) = nansum(data_test.Var5(i:(7*wks_back):end));
        total_US_Mean_2(i) = nansum(mpgMean_2(i:(7*wks_back):end));
        
        total_US_GT_3(i) = nansum(data_test.Var6(i:(7*wks_back):end));
        total_US_Mean_3(i) = nansum(mpgMean_3(i:(7*wks_back):end));
        
        total_US_GT_4(i) = nansum(data_test.Var7(i:(7*wks_back):end));
    ...
    fullpath = [pathname];
    writetable(T_all, [fullpath '/' thisdate '-SGroup-LSBoosted.csv']);
end