function [beta_all_cell, fittedC, ci] = var_ind_beta_un_COVID(data_4, passengerFlow, alpha_l, k_l, un_fact, popu, jp_l, wi, ret_conf, compute_region, window_size, extra_imm)
    
    maxt = size(data_4, 2);    
    
    season_start_day = maxt - wi;
    ...
    for j=1:length(popu)
        ...
        if season_start_day + 15 < maxt
            skip_days = season_start_day;%maxt - window_size(j); 
        else
