...
seasons_back = 4; %1 starts at 2019-2020
...
for wks_back = x:-1:5
    data_train = table;
    data_test = table;
    for i=1:(size(old_data_no20202,1)/(239*(11-seasons_back + 1))) %Basically looping over states
...
%     RMSE_4(:,27-wks_back) = sqrt(nanmean((GT_4 - pred_4_m).^2)); 
    ...
    writetable(T_all, [fullpath '\' int2str(year) '/' int2str(season) '-' int2str(MMR_WEEK) '-SGroup-LSBoost.csv']);
end