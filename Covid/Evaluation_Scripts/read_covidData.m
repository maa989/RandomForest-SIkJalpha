function [flu_data_m, flu_data_q] = read_covidData(path)
%PR72->52. VI78->3. US->14. WYOMING56->7. WISCONSIN55->43
    Q = [0.010 0.0250 0.050:0.050:0.950 0.975 0.990];
    flu_data_m= nan(54,28);
    flu_data_q = nan(54,23,28);
    data = readtable(path);
    data = data(contains(data.target, 'inc hosp'),:);
    ...
    for i = 1:size(data_mean,1)
        ...
        elseif(data_mean.location(i) >=57)
            continue
        else
    ...
    for i = 1:size(data_quantiles,1)
        ...
        elseif(data_quantiles.location(i) >=57)
            continue
        else
